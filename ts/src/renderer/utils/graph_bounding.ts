...
function calculateDynamicBounds(
    x: number, y: number, ratio: number, posInfo: types.PosInfoArray
...
function findYExtremes(x: number, posInfo: types.PosInfoArray): { minY: number, maxY: number } {
    let minY: number, maxY: number, buff: number;
...
function getCenterCoords(
    renderer: types.SigmaRenderer, container: types.Div | types.DivRef
): { x: number, y: number } {
    // Checking if ref type
    const div: types.Div = 'current' in container ? container.current : container;

    return renderer.viewportToGraph({ x: div.offsetWidth / 2, y: div.offsetHeight / 2 });
}


function getVisibilityRanges(
    camera: types.Camera, renderer: types.SigmaRenderer, container: types.Div | types.DivRef
...
export default { calculateDynamicBounds, getVisibilityRanges };
